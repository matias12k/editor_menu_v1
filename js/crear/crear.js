...
function createButton(label) {
    ...
    if (button.innerHTML === 'Eliminar Botón' || button.innerHTML === 'Eliminar Submenu') {
        ...
        icono.src = '../../imagenes/crear/Icono-Basurero.jpeg';
...
document.addEventListener('DOMContentLoaded', function() {
    // Obtener el botón que modifica el texto del menú y el contenedor de estilos
    var btnModificarTextoMenu = document.getElementById('btnModificarTextoMenu');
    var estilos = document.getElementById('estilos');

    // Verificar que los elementos existen antes de añadirles eventos
    if (btnModificarTextoMenu && estilos) {
        // Añadir un evento de clic al botón
        btnModificarTextoMenu.addEventListener('click', function() {
            // Mostrar el contenedor de estilos al hacer clic
            estilos.style.display = 'block';

            // Cargar el contenido del archivo editar_texto.php
            fetch('../editar_texto/editar_texto.php')
                .then(response => response.text())  // Convertir la respuesta a texto
                .then(data => {
                    // Crear un div temporal para analizar el contenido HTML
                    ...
                    if (estilosDiv) {
                        // Reemplazar el contenido del div 'estilos' con el contenido cargado
                        estilos.innerHTML = estilosDiv.innerHTML;

                        // Obtener y añadir un evento al botón de cerrar menú, si existe
                        var cerrarMenuBtn = estilos.querySelector('#cerrarMenu');
                        if (cerrarMenuBtn) {
                            cerrarMenuBtn.addEventListener('click', function() {
                                // Ocultar el contenedor de estilos al hacer clic en el botón
                                estilos.style.display = 'none';
                            });
                        } else {
                            // Mensaje de error si el botón no se encuentra
                            console.error('No se encontró el botón cerrarMenu en el contenido cargado.');
                        }

                        // Verificar y ejecutar la función CambiarTamanioTexto si el elemento selectTamanio está presente
                        var selectTamanio = estilos.querySelector('#selectTamanio');
                        if (selectTamanio) {
                            CambiarTamanioTexto();
                        } else {
                        ...
                        console.error('No se encontró el div con ID "estilos" en la respuesta');
                    }
                })
                .catch(error => {
                    // Mensaje de error si hay problemas al cargar el contenido
                    console.error('Error al cargar el contenido:', error);
                });
        });
    } else {
        // Mensaje de error si los elementos necesarios no se encuentran en el DOM
        console.error('Elementos no encontrados: btnModificarTextoMenu o estilos');
    }
});
